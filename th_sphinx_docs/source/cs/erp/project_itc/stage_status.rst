...
The following subtypes are triggered on ``project.task``:
 ...
 - ``th_mt_task_new``: new tasks. Condition: ``obj.stage_id and obj.stage_id.sequence == 1``
 - ``th_mt_task_stage``: stage changed. Condition: ``obj.stage_id and obj.stage_id.sequence != 1``
 - ``mt_task_assigned``: user assigned. condition: ``obj.user_id and obj.user_id.id``
 - ``th_mt_task_blocked``: kanban state blocked. Condition: ``obj.kanban_state == 'blocked'``
